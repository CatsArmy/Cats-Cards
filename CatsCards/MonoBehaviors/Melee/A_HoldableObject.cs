...
namespace Lightsaber
{
    public class A_HoldableObject : MonoBehaviour
    {
        ...
        internal static void MakeGunHoldable(int playerID, bool holdableObj)
        {
            ...
            //GameObject HoldableObj = springObj.transform.Find($"{weapon.name}(Clone)")?.gameObject;//object.name?+"(Clone)"
            if (HoldableObj is null)
            {
                HoldableObj = Instantiate(CatsCards.assets.LoadAsset<GameObject>("Lightsaber"), springObj.transform);
                //HoldableObj = Instantiate(CatsCards.assets.LoadAsset<GameObject>($"{weapon.name}"), springObj.transform);
                ...
                //ObjectCollider objCollider = HoldableObj.transform.GetChild(0).gameObject.GetOrAddComponent<ObjectCollider>();
        ...
        private void OnDestroy()
        {
            ...
            GameObject springObj = gun.transform.Find("Spring").gameObject;
            GameObject HoldableObj = springObj.transform.Find($"Lightsaber(Clone)")?.gameObject;
            //GameObject HoldableObj = springObj.transform.Find($"{weapon.name}(Clone)")?.gameObject;
            if (HoldableObj is not null)
