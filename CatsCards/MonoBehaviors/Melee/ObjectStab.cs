...
namespace Lightsaber
{
    /*class ObjectStab : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (this.Player.data.view.IsMine /*&& this.HoldableObj.SwitchTimer <= 0f*//* && this.StabTimer <= 0f && this.Input.shootWasPressed && this.HoldableObj.IsOut)
            {
                this.DoStab();
            }
        }
    ...
    class ObjectSlash : MonoBehaviour
    {
       ...
       // private float sinceBlade = slashCooldown;
        ...
        internal void TrySlash(Collision2D collision)
        {
            ...
            CatsCards.instance.ExecuteAfterSeconds(slashAnimDuration, () => { hitColliders.Remove(collider2D); });
